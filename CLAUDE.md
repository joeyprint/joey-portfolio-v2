...
- **`src/pages/`** — Routing only. A page file assembles containers inside `BaseLayout` and owns the page-level chrome (nav, hero, `<style is:global>` for page-specific utilities). No component-level markup, no business data.
- **`src/containers/`** — Business logic layer. Each container owns its section data (arrays, snippets, computed values), composes `components`, and renders a self-contained section. Containers have business meaning — they know _what_ to display and _why_.
- **`src/components/`** — Pure presentational primitives. Components receive data via props/slots, apply styling, and know nothing about the page or section context. No hardcoded content, no page-specific logic.

- `src/pages/` — Astro file-based routing. Three pages: `index.astro` (homepage), `projects.astro`, and `component-library.astro`. Pages assemble `containers` inside `BaseLayout`.
...
- `src/containers/` — Section-level components that compose a page section out of `components`. Organized by page: `homepage/` (`HeroBanner`, `CompanyList`, `Achievements` = Mastery Results section, `HighlightProjects` = Excellence Projects section, `QuoteSection`, `ContactMe`), `projects/` (`projectList.astro`), and `component-library/` (`CLColors`, `CLTypography`, `CLRadii`, `CLMotion`, `CLButtons`, `CLTag`, `CLWorkCard`, `CLSideCard`, `CLProjectItem`, `CLAchievement`).
- `src/components/` — Reusable UI primitives. Buttons/links (`Button`, `IconButton`, `Link`), content blocks (`Container`, `ProjectItem`, `Achievement`, `CompanyBox`), project-page cards (`Tag`, `LiveBadge`, `WorkCard`, `SideProjectCard`), the dark-mode `Toolbar`, a Swiper-based `Carousel/`, and `icons/` (one `.astro` per icon, e.g. `SoftwareDevelopmentIcon`, `HeartIcon`, `FolderFilledIcon`, `FacilitateIcon`, `GitHubIcon`, `LinkedInIcon`, `DownloadOutlinedIcon`, `MediumIcon`, `HamburgerMenuIcon`, `CloseIcon`). `Button` uses `class-variance-authority` (`cva`) for variant/size/color composition and exposes `start-icon`/`end-icon` named slots. `WorkCard` accepts a `mark` prop (text abbreviation or image URL) and an optional `imageUrl` for the 16:9 thumbnail; `tint` sets the fallback background when no image is provided. `SideProjectCard` wraps an `<a>` and accepts a `thumbnail` named slot for custom thumb content.
